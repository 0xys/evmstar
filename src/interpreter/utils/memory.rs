...
pub fn mload(offset: U256, memory: &mut Memory, stack: &mut Stack, gas_left: i32) -> Result<i32, StatusCode> {
    Ok(0)
}

/// store the top item of the stack into memory at `offset`.
/// any padding occurred will incur gas cost.
pub fn mstore(offset: U256, memory: &mut Memory, stack: &mut Stack, gas_left: i32) -> Result<i32, StatusCode> {
    Ok(0)
}

/// store the top item of the stack into memory at `offset`.
/// any padding occurred will incur gas cost.
pub fn mstore8(offset: U256, memory: &mut Memory, stack: &mut Stack, gas_left: i32) -> Result<i32, StatusCode> {
    Ok(0)
}